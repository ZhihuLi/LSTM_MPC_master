...
class LSTM_Width():
    ...
    def save(self):
        with self.sess1.as_default():
            with self.graph.as_default():
                ...
                saver.save(self.sess1, './Data_Width/lstm_params', write_meta_graph=True)
    ...
    def restore(self):
        ...
        with self.sess1.as_default():
            with self.graph.as_default():
                ...
                saver = tf.train.Saver()
                saver.restore(self.sess1, './Data_Width/lstm_params')
    ...
    def eval(self):
                predictions = []
                labels = []
                for i in range(self.TEST_LENGTH - self.TIMESTEPS):
                    X = self.test_X[[i]]
                    y = self.test_y[[i]]
                ...
                predictions = predictions *0.86990533 +6.33300273
                labels = labels * 0.86990533 +  6.33300273
                ...
                np.savetxt('./Data_Width/eval_result.txt', eval_result)
                plt.figure()
    ...
    def input_model_data(self, number):
        ...
        #     welding_data = np.loadtxt('D:\\研究生\\课题研究\\焊道形状控制\\实验数据\\原始数据\\2020年4月\\处理结果\\数据\\Height_Width_Bead'+str(num)+'.txt')
        ...
        for num in range(12, 40):
            ...
            training_examples_output = raw_data_output[650:1600, 2]
            training_examples_output = (training_examples_output - mu[3]) / sigma[3]
            self.train_X, self.train_y = self.model_data_pack(training_examples_input, training_examples_output,
        ...
        for num in range(12, 40):
            ...
            training_examples_output = raw_data_output[650:1600, 2]
            training_examples_output = (training_examples_output - mu[3]) / sigma[3]
            self.test_X, self.test_y = self.model_data_pack(training_examples_input, training_examples_output,
